    ...
    override fun onCreate(savedInstanceState: Bundle?) {
        ...
        val jsonArrayRequest = JsonArrayRequest(
            ...
            Response.Listener { response ->
                ...
                // Agregar listener al SwipeRefreshLayout para realizar la actualización
                swipeRefreshLayout.setOnRefreshListener {
                    // Realizar nuevamente la solicitud a la API para actualizar los datos
                    val jsonArrayRequestRefresh = JsonArrayRequest(
                        Request.Method.GET, url, null,
                        Response.Listener { responseRefresh ->
                            val empresaItemsRefresh = parseEmpresaItems(responseRefresh)
                            adapter.clear()
                            adapter.addAll(empresaItemsRefresh)
                            adapter.notifyDataSetChanged()

                            swipeRefreshLayout.isRefreshing = false // Indicar que la actualización ha finalizado
                        },
                        Response.ErrorListener { errorRefresh ->
                            // Manejar errores de la solicitud de actualización
                            swipeRefreshLayout.isRefreshing = false // Indicar que la actualización ha finalizado
                        }
                    )

                    requestQueue.add(jsonArrayRequestRefresh)
                }
            },
